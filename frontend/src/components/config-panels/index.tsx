      ...
      <div className="flex flex-col h-full bg-linear-to-b from-transparent to-muted/10">
        <header className="p-5 border-b border-border/40 flex flex-row items-center justify-between space-y-0 shrink-0">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 rounded-[12px] hover:bg-muted/80 transition-all active:scale-95"
        ...
        <nav className="p-4 shrink-0">
          <div className="flex p-1 bg-muted/40 border border-border/40 rounded-[20px] w-full">
            <button
              onClick={() => setActiveTab('config')}
              className={cn(
                "flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-bold uppercase tracking-widest transition-all duration-300 rounded-[14px]",
                activeTab === 'config'
                  ? "bg-background text-foreground dark:bg-white/10"
          ...
          {activeTab === 'config' ? (
            <div className="space-y-6">
              <div className="space-y-2.5">
                ...
                <Input
                  id="node-name"
                  className="bg-muted/20 border-border/80 dark:border-white/10 dark:bg-white/5 focus:bg-muted/30 focus:border-primary/40 transition-all rounded-[20px] h-10 px-4 text-sm"
              ...
              <div className={cn(
                ...
                selectedNode.data.has_breakpoint
                  ...
                  : "bg-muted/20 dark:bg-white/5 border-border/80 dark:border-white/10"
              )}>
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <Label htmlFor="breakpoint" className={cn(
                      "text-[10px] font-black uppercase tracking-[0.2em]",
                      selectedNode.data.has_breakpoint ? "text-amber-500" : "text-muted-foreground/50"
                    )}>Breakpoint</Label>
              ...
              <Button
                variant="destructive"
                className="w-full h-11 font-bold tracking-tight rounded-[20px] shadow-none hover:-translate-y-px active:translate-y-0 transition-all"
                onClick={() => {
